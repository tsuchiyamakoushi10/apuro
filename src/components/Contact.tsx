    ...
    <Section id={id}>
      <div className="wrap">
        <div className="grid grid-cols-2 gap-6 max-[960px]:grid-cols-1">
          ...
          >
            <span>
              <span className="block font-heading text-[19px] text-blue-ink">お電話でのご相談</span>
...
export function FormCard({
  ...
  const inner = (
    <>
      <span>
        <span className="block font-heading text-[19px] text-blue-ink">{label}</span>
        ...
        {!ready ? <span className="mt-2 block text-[12px] tbd">{href}</span> : null}
      </span>
      {ready ? <Arrow /> : null}
    </>
  );
