...
3. **原稿はすべて `docs/アプロ訪問看護_HP原稿.xlsx` が正。** 文言を勝手に書き換えたり、要約したり、「〜します！」のような調子に直したりしない。改行位置も原稿の意図を尊重する。
   ただし **960px以下は改行位置を捨てて流し込む**（`Lines` の `reflow`。本文の `Paragraphs` は既定で流す）。
   原稿の1行は40字前後あり、スマホの幅では途中で折り返して2〜3字だけが次の行に残るため。
   改行が値の区切りになっている箇所（募集要項の表）では流さない。
4. **セクションごとに面の色を変えない。**白を基調にして、色はアクセントに抑える（ベース70：メイン25：アクセント5）。
   1ページで面に色を敷くのは1〜2箇所まで。それ以外は罫線と余白で区切る。
   項目が多いページは `<details>` で畳む（`/recruit` の `Disclosure`）。開いたまま並べると文字の壁になる。
5. **モックにない装飾を足さない。** 影とグラデーションは使わない。ホバーでカードを浮き上がらせない（`translateY` も影も付けない）。動きは「モーション」の節に書いたものだけにする。
   例外はヒーローの `.hero-scrim` だけ。実写真の上で見出しが人物に重なって読めなくなるため、可読性のために置いている。装飾ではないので消さないこと。
6. 採用ページを実装する際は `docs/spec.md` の職業安定法チェックリストを必ず確認する。
...
--blue-ink:#123A5C; /* 見出し */
--blue-soft:#DCEAF3;
--sand:#DFC9A5;     /* TOPの採用バナーの英字ラベルとボタンだけ。面には敷かない */
...
- コンテンツ幅は `1160px` / 左右 `32px`（`.wrap`）。ヘッダーだけ1380px以上で `1440px` / 左右 `40px`（`.wrap-header`）
- ヘッダーのナビと電話番号は **1380px未満で畳む**（サイト全体のブレークポイントは960px。ここだけ例外）。
  ロゴ256px＋ナビ441px＋電話・ボタン541px＋余白で1行に1299px要る。
  1380px の画面でコンテナに使えるのは 1300px なので、ここが下限になる。
  **ナビを増やしたらこの数字が動く。**「お知らせ」を足したときはナビが339→441pxになり、
  ブレークポイントを1300→1380pxに上げた。Chromiumで測り直して合わせること。
  畳んだ先は MENU（`MobileMenu`）で、電話番号もその中に入っている。
  開くとヘッダーの下が淡い青で埋まり、大きな文字のリンクと、電話・フォームをまとめた青い面が出る。
  下に押し広げない。遷移したら閉じる必要があるので、ここだけクライアントコンポーネントにしてある。
...
例外はアイコン（`src/components/icons.tsx`）。`/about` の行動指針（`ValueIcon`）、
`/service` の「私たちが行うこと」（`ServiceIcon`）、`/recruit` の福利厚生（`BenefitIcon`）に置いている。
写真を減らした代わりに置いている。塗りなしの一本線（`viewBox="0 0 24 24"` / `stroke-width:1.3`）で、
共通モチーフの面（`--blue-soft`）の中に載せる。面の形は増やさない。
面の大きさは2種類だけ。節の見出しに並ぶものは64px、項目に並ぶものは48px。
アイコンを足すときはこの作りに揃える。参照は <https://happywood.or.jp/vision/>。

`/service` の流れも同じモチーフの面で図にしている（`.flow-no`）。線でつなぐだけで、新しい形は足さない。

対応エリアだけは実際の地図（`public/images/tokyo-map.svg`）。国土数値情報から起こしていて、
作り直しは `scripts/make-tokyo-map.py`。**エリアを変えたら地図も作り直す。出典表記は消さない。**
...
| `/recruit` | メイン画像／採用の特徴（理念・働き方・教育・人物像・正直に伝えること）／福利厚生／募集要項／エントリー | 05・06 |
| `/news` | お知らせ一覧 | `src/content/news.ts`（CMSなし） |
...
`/service` は冒頭のメインビジュアルと特徴01〜04に1枚ずつ（概要からは外した）。
`/recruit` は冒頭のメインビジュアル1枚だけ。
ページをまたいだ転用は許容しているが、**メインビジュアルはページごとに別の写真にする。**
...
`site.mapEmbedSrc` が `【調整中】` のあいだは枠だけを出す（`.ph`）。写真の枠はすべて実写真に入れ替わっていて、
プレースホルダが残っているのはここだけ。
