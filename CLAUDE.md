...
リンクのカードはホバーでその面に重なるところまで動く（リンクでないカードは敷くだけ）。参照：umicahi.com

面の色は地の色より一段濃い青にする。クラスで切り替える。

| 地 | クラス | 面の色 |
|---|---|---|
| 白 | `.offset-card` のみ | `--blue-soft` |
| 淡い面（`--mist`） | `.offset-on-mist` | `--blue` を20%混ぜた色 |
| 青ベタ面 | `.offset-on-color` | 白の22% |

淡い面の上で `--blue-soft` を使わないのは、`--mist` と差が付かず面が見えなくなるため。

敷いてあるのは、TOPの特徴・こんなときに、`/about` の行動指針、`/service` のこんなときに・特徴、
`/recruit` の人物像・働き方、電話とフォームのカード。罫線で区切っているリストや表には敷かない。

番号は共通モチーフの形をしたバッジ（`.badge-no`）に入れる。TOPの特徴、`/about` の行動指針、
`/service` の特徴で使う。連番のリスト（ご利用の流れ）は小さい英字のままにする。
