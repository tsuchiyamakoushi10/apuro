...
PCだけ一段大きくしており（本文15px→17px、見出し29px→33px）、`px` で書くと追従しない。
モックの値を `rem` に直すときは16で割る（13.5px → `0.84375rem`）。ヒーローの文字とヘッダーのロゴも
`rem` にしてあり、PCでは一緒に大きくなる（キャッチ47→53px、ロゴ18→20px）。
`px` のまま残っているのは 600px 以下・430px 以下のロゴ縮小指定だけで、これはスマホ専用のため追従させる必要がない。
スマホ（960px以下）はモックの値のまま変えていない。
